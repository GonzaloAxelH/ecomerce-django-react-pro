...
class ProductDetailView(APIView):
    ...
    def get(self, request, productId, format=None):
        ...
        if Product.objects.filter(id=product_id).exists():
            ...
            return Response({"error": "Product ID no existe"}, status=status.HTTP_404_NOT_FOUND)
...
class ListProductView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if products:
            ...
            return Response({"Error": "no products List"}, status=status.HTTP_404_NOT_FOUND)
...
class ListRelatedView(APIView):
    ...
    def get(self, request, productId, format=None):
        try:
            ...
            return Response({"error": "Product ID must be a integer"}, status=status.HTTP_404_NOT_FOUND)

        # si existe el productid
        if not Product.objects.filter(id=product_id).exists():
            return Response({
                "error": "Product with this product ID does no exist"
            }, status=status.HTTP_404_NOT_FOUND)
        ...
        if Product.objects.filter(category=category).exists():
            ...
            if len(related_products.data) > 3:
                ...
                return Response({"empty": "No related products Found"}, status=status.HTTP_200_OK)
        else:
